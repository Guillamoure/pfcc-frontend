...
import PatchNotes from "../components/modals/patch_notes";
...
const ModalSkeleton = props => {
  const renderComponent = modal => {
    ...
    switch (m) {
      ...
      case "feat":
        return (
          <Feat feat={modal.obj} exitModal={exitModal} clickOut={clickOut} />
        );
      case "statBlock":
        return (
          <CreatureStatBlock
      ...
      default:
        return (
          <GenericDescription
            name={modal.obj.name}
            description={modal.obj.description}
          />
        );
    }
  };
  ...
  React.useEffect(() => {
    if (sidebarContainer.current) {
      ...
      if (
        exitStyle &&
        exitStyle.width !== tabStyleDuplicate.width &&
        exitStyle.bottom !== tabStyleDuplicate.bottom
      ) {
        setExitStyle(tabStyleDuplicate);
      }
      // infinite loop if exitStyle is included in the dependency array
    }
  }, [exitStyle, sidebarContainer]);
  ...
  React.useEffect(() => {
    ...
    setExitStyle({
      bottom: `${window.innerHeight / bottomDenominator - 4}px`,
  ...
  const displayAllTabs = () => {
    return props.modal.map((m, i) => {
      ...
      let tabStyle = {
        ...
        borderColor: `#${props.settings.borderColor}`,
      ...
      let toggleButton = (
        <button onClick={() => updateToggleButton(i)}>
          <FontAwesomeIcon icon={collapsed ? faChevronUp : faChevronDown} />
        </button>
      );
      let embiggenButton = (
        <button onClick={() => toggleBig(!big)}>
          <FontAwesomeIcon icon={big ? faCompress : faExpand} />
        </button>
      );
      return (
        <>
          <div id="sidebar-exit" style={tabStyle}>
            <span
            ...
            <span style={{ textAlign: "center" }}>
              {toggleButton}
              {cancelButton}
              {i === activeIndex && !collapsed && embiggenButton}
            </span>
          </div>
